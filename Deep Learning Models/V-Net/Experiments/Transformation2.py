...
class Experiment_transformation():
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, self.n_epochs + 1):
            ...
            print(f'Epoch {epoch} Train loss {avg_train_loss:.4f} Train Dice {avg_train_dice:.4f} Train std {std_train_dice:.4f}')
            ...
            if epoch % self.val_interval == 0:
                ...
                avg_val_loss = val_epoch_loss / len(self.val_loader.dataset)
                avg_val_dice = torch.mean(torch.stack(val_dice_scores)).item()#####################
                std_val_dice = torch.std(torch.stack(val_dice_scores)).item()

                print(f'Epoch {epoch} Validation loss {avg_val_loss:.4f} Validation Dice {avg_val_dice:.4f} Validation std {std_val_dice:.4f}')
                ...
                avg_test_loss = test_epoch_loss / len(self.test_loader.dataset)
                avg_test_dice = torch.mean(torch.stack(test_dice_scores)).item()
                std_test_dice = torch.std(torch.stack(test_dice_scores)).item()

                print(f'Epoch {epoch} Test loss {avg_test_loss:.4f} Test Dice {avg_test_dice:.4f} Test std {std_test_dice:.4f}')
